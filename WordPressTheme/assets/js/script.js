...
jQuery(function ($) {
  ...
  $(function () {
    ...
    if (pram && $('[data-tab="' + pram + '"]').length) {
      ...
      $('.tab-button[data-tab="' + pram + '"] .tab-button__link-icon').addClass("tab-button__link-icon--active");
      $('.tab-panel[data-tab="' + pram + '"]').addClass("tab-panel--active");
    }

    // ロード後のタブ切り替え
    $(".tab-button").on("click", function () {
      var dataTab = $(this).data("tab"); // data-tab属性の値を取得
      $(".tab-button").removeClass("tab-button--active");
      $(".tab-panel").removeClass("tab-panel--active");
      $(".tab-button__link-icon").removeClass("tab-button__link-icon--active");
      $(this).addClass("tab-button--active");
      $(this).find(".tab-button__link-icon").addClass("tab-button__link-icon--active");
  ...
  $(".article-archive__item-btn").click(function () {
    var parentItem = $(this).closest(".article-archive__item");
  ...
  if (document.querySelector(".js-campaign")) {
    var campaignSwiper = new Swiper(".js-campaign", {
      spaceBetween: 24,
      centeredSlides: true,
      // Optional parameters
      loop: true,
  ...
  function PageTopAnime() {
    ...
    if (scroll + wH >= footerPos + 10) {
      var pos = scroll + wH - footerPos + 20; //スクロールの値＋画面の高さからfooterの位置＋40pxを引いた場所を取得し
  ...
  $(".faq-accordion__header").on("click", function () {
    ...
    var isOpen = $target.hasClass("faq-accordion__header--is-open");
    if (isOpen) {
      // アコーディオンを閉じる
      $target.removeClass("faq-accordion__header--is-open");
      $panel.addClass("faq-accordion__panel--is-close").removeClass("faq-accordion__panel--is-open");
    } else {
      // アコーディオンを開く
      $target.addClass("faq-accordion__header--is-open");
      $panel.removeClass("faq-accordion__panel--is-close").addClass("faq-accordion__panel--is-open");
    }
  });
});
