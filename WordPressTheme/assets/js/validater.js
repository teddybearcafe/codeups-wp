...
window.addEventListener("DOMContentLoaded", function () {
  ...
  var submitButton = form.querySelector(".button--consent");
  function validateForm() {
    var name = form.querySelector("[name='name']");
    var phone = form.querySelector("[name='phone']");
    var email = form.querySelector("[name='email']");
    var campaign = form.querySelector("[name='campaign']");
    var message = form.querySelector("[name='message']");
    var checkboxes = form.querySelectorAll("[name='contact-type[]']");
    var consent = form.querySelector("#consent1");
    var submitBtn = form.querySelector(".button--consent");
    var consentLabel = form.querySelector(".contact-form__form-label--consent");
    var emailPattern = /^[^@\s]+@[^@\s]+\.[^@\s]+$/; //xxx@xxxx.xxxxの形式であることを確認
    var phonePattern = /^\d{2,4}-?\d{2,4}-?\d{4}$/; //xxx-xxxx-xxxx（ハイフン無しに対応）
    var postalCodePattern = /^\d{3}-?\d{4}$/; //xxx-xxxx（ハイフン無しに対応）
    ...
    var labels = form.querySelectorAll(".contact-form__form-label:not([for='consent1'])");
    ...
    if (name.value.trim() === "" || phone.value.trim() === "" || email.value.trim() === "" || message.value.trim() === "" || !isChecked || !consent.checked) {
